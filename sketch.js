...
if(gameState === "HOME"){
    ...
    stroke("black");
    textSize(30);
    text("Press 0 to See instructions For the game",width/2 - 200,height/2 - 200);
    stroke("white")
    textSize(30);
...
if(gameState === "INTS"){
    ...
    textSize(30);
    fill("black")
    text("Note: If you prefire and time it so that you hit the zombie the zombie maynot apear to spawn \n As hitboxes are loaded first and then image ",width/2 - 600,height/2 - 300);
    textSize(30);
    fill("white")
    text("Press R to reload during the game",width/2 - 200,height/2 - 100);
    text("No errors there as i am the best",width/2 - 200,height/2);
    text("Use the arrow keys to move the player",width/2 - 200,height/2 +100);
    text("Fun Fact:You know this crazy thing? no?Press ESCAPE TO GO TO HOME SCREEN",width/2 - 300,height/2 + 200);
    if(keyDown(ESCAPE)){
        gameState = "HOME";
        background(homeScreenImg);
    }
...
if(gameState === "PLAY1"){
    textSize(32)
    text("Get 10 score to win!",width/2 - 200,height/2 - 200);
...
if(gameState === "PLAY2"){
    textSize(32)
    text("Get 25 score to win!",width/2 - 200,height/2 - 200);
    if(playerMoving == false){
        player.changeImage('walk');
    }else{
        player.changeAnimation('player');
    ...
    if(gameState === "PLAY3"){
        textSize(32)
        text("Get 50 score to win!",width/2 - 200,height/2 - 200);
        if(playerMoving == false){
            player.changeImage('walk');
        }else{
            player.changeAnimation('player');
